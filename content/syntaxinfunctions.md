...
在函数的where部分定义的常量只能这个函数可见，所以不用担心会污染其他函数的命名空间。注意这些常量都排列在同一列。如果不适当地排列这些常量声明，Haskell将没办法知道它们都属于同一个代码块。

_where_绑定在函数体中的不同模式不能共享，如果需要在一个函数的几个模式中共享一些变量，需要在全局定义它们。

甚至可以在**模式匹配**中使用where绑定，可以将前一个函数的where部分写成这样：

    ...  
    where bmi = weight / height ^ 2  
          (skinny, normal, fat) = (18.5, 25.0, 30.0)

下面让我们创建一个简单的函数，它接受一个人和姓和名并返回姓名的缩写。
...
其实也可以直接在函数的参数部分使用模式匹配（实际上会更简短清晰），但这个例子只是展示如果在where绑定中使用它。

就如在where块中定义常量一样，还可以定义函数。下面看看这个函数，接受一个身高和体重的列表并返回一个ＢＭＩ的列表。

    calcBmis :: (RealFloat a) => [(a, a)] -> [a]  
    calcBmis xs = [bmi w h | (w, h) <- xs]  
        where bmi weight height = weight / height ^ 2

需要在函数中引入`bmi`函数的原因是我们无法通过函数的参数来计算一个ＢＭＩ。我们需要检查参数列表并对列表中的每一个对求ＢＭＩ。

_where_绑定还可以嵌套，通常在会为一个函数在其_where_块中定义一些帮助函数，然后在这些帮助函数的where块中定义一些帮助函数。

### Let绑定

与where绑定非常类似的是let绑定。where绑定是这样一种语法结构，你可以在函数末尾绑定一些常量并在整个函数体，包括守卫中看到它们。let绑定自身也是表达式，让你可以在任何地方绑定常量，但是不能扩展到其他守卫。和Haskell中其他将值绑定到名字的语法结构一样，let绑定也可以用于模式匹配。
