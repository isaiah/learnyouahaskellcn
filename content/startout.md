...
从某种意义上说元组和列表很相像，都可以用来把多个值存储在一个值内。然而，它们之间也有一些根本的差别。列表内的元素必须相同类型，并且长度没有限制。但是使用元组时应该确切地知道元组的长度，和各组成部分的类型。元组用括号表示，元素通过逗号分隔。

另一个差别是元组不必是同质的。与列表不同，元组可以由不同类型的元素组成。

假设现在需要用Haskell表示一个两维向量。一种方式是用列表，如果用一些向量组成的列表来表示一个两维平面，可以用`[[1,2],[8,11],[4,5]]`表示。这种方式的问题在于Haskell无法阻止`[[1,2],[8,11,5],[4,5]]`这样无意义的列表。但是一个长度为2的元组（也称为对）有它自己的类型，这样一个列表就不能同时包含二元组和三元组（长度为3的元组），所以这种情况可以使用二元组。向量用圆括号而不是方括号包围：`[(1,2),(8,11),(4,5)]`。如果想要表示一个`[(1,2),(8,11,5),(4,5)]`这样的形状怎么办呢？我们会收到一个这样的错误：
...
错误信息的意思是我们尝试在同一个列表中包含二元组和三元组，这是不应该发生的。也不能构建像`[(1,2),("One",2)]`这样的元组，因为列表的第一个元素是一个组成的二元组，而第二元素却是一个由一个字符串和一个数字组成的二元组。元组能用来表示广泛的数据类型。例如可以用三元组`("Christopher", "Walken", 55)`来表示一个人的名字和年龄。从这个例子可以看出，元组也可以包含列表。

当你事先知道数据结构的组成部分的时候可以使用元组。元组相对来说结构更加刻板，因为不能长度的元组都有其自己的类型，所以不能写一个通用的函数来给元组追加一个元素。必須为二元组，三元组和四元组写一个各自独立的函数来为其末尾追加元素。

虽然存在单元素列表，但是却不存在一元组。仔细想想就会发生一元组似乎并没有什么用处。

和列表一样，元件可以比较的元组也可以进行比较。不同的是不能比较不同长度的元组，但是可以比较不同长度的列表。下面是两个操作操作二元组的有用的函数：
...
> 注意这两个函数只能用于二元组。我们将在后面介绍从多元组中抽取数据的方法。

`zip`函数可以产生一个二元组的列表。它将两个列表整合为一个列表并将列表中位置相对应的元素组合成二元组。这是一个简单的函数但是却有很大的作用。当你想要以某种方式将两个列表组合在一起或者想要同时遍历两个列表时尤其有用。如下面的例子。
...
它将各自的元素组成对并产生一个新的列表。因为二元组可以包含不同类型的元素，所以`zip`可以接受两个不同类型的列表作为参数并将两们拴在一起。如果作为参数的两个列表长度不同会怎么样呢？

        ghci> zip [5,3,2,6,2,7,2,5,4,6,6] ["im","a","turtle"]  
        [(5,"im"),(3,"a"),(2,"turtle")]

较长的列表被截断以适应较短的列表。因为Haskell是惰性的，所以可以将有穷的列表和无穷的列表整合起来。

        ghci> zip [1..] ["apple", "orange", "cherry", "mango"]  
        [(1,"apple"),(2,"orange"),(3,"cherry"),(4,"mango")]

![abc](http://s3.amazonaws.com/lyah/pythag.png)

这是一个涉及元组和列表集合的问题：求一个满足以下条件的直角三角形，所有边长都是整数，并且所有边长都小于等于10，同时周长等于24。首先让我们列举所有边长都小于等于10的三角形：

        ghci> let triangles = [ (a,b,c) | c <- [1..10], b <- [1..10], a <- [1..10] ]

只是从三个列表中各抽取一个元素，然后输出函数将它们组成成三元组。如果在GHCI中查看`triangles`可以得到所有边长小于等于10的可能的三角形。然后增加约束条件使所有三角形都为直角三角形。我们修改这个函数以使边b不超弦的长度，边a的长度不超过边b。

        ghci> let rightTriangles = [ (a,b,c) | c <- [1..10], b <- [1..c], a <- [1..b], a^2 + b^2 == c^2]


几乎要完成了。现在修改这个函数以使三角形周长为24。

        ghci> let rightTriangles' = [ (a,b,c) | c <- [1..10], b <- [1..c], a <- [1..b], a^2 + b^2 == c^2, a+b+c == 24]  
        ghci> rightTriangles'  
        [(6,8,10)]

这就是答案！这也是函数式编程的一般模式。从一个结果集开始，对这个结果集进行转换和过滤直到得到正确答案。
